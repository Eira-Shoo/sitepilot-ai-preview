...
export function TrustSection({ section }: { section: Trust }) {
  ...
  return (
    <section className="border-b border-border/40 bg-muted/15 py-14 sm:py-16">
      <div className={SECTION_CONTAINER}>
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-2 sm:gap-5 lg:grid-cols-4">
          {items.map((item: string, i: number) => (
            <div
              key={i}
              className="flex items-center gap-4 rounded-2xl border border-border/50 bg-card/80 px-5 py-5 shadow-sm sm:px-6"
            >
              {i === 0 ? (
              ...
              <p className="text-base font-medium leading-relaxed sm:text-[1.0625rem]">{item}</p>
            </div>
          ))}
        </div>
      </div>
