...
async function generateBlueprintFromOpenAi(
  ...
  let validated = safeParseWebsiteBlueprint(parsed);
  if (!validated.success) {
    ...
    validated = safeParseWebsiteBlueprint(parsed);
    if (!validated.success) {
 ...
 * - NEXT_PUBLIC_DEMO_MODE=1 or missing API key → deterministic mock.
 * - Otherwise → OpenAI, with optional mock fallback when allowMockFallback is true.
 */
