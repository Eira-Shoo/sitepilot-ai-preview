...
async function upsertEnv(projectId, key, value, type = "encrypted") {
  const envs = (await listEnv(projectId)).filter((e) => e.key === key);

  if (envs.length) {
    for (const env of envs) {
      await vercel(`/v9/projects/${projectId}/env/${env.id}`, {
        method: "PATCH",
        body: JSON.stringify({ value, target: TARGETS }),
      });
    }
    console.log(`  ✓ ${key} updated (${envs.length} existing)`);
    return;
  }
...
async function redeploy(projectId) {
  ...
  if (!latest?.uid) {
    console.log("No deployment found; push to GitHub to deploy.");
    return;
  }
  await vercel(`/v13/deployments/${latest.uid}/redeploy`, {
    method: "POST",
    body: JSON.stringify({}),
  });
...
async function main() {
  ...
  await upsertEnv(projectId, "NEXT_PUBLIC_DEMO_MODE", env.NEXT_PUBLIC_DEMO_MODE, "plain");
  await upsertEnv(projectId, "NEXT_PUBLIC_APP_URL", env.NEXT_PUBLIC_APP_URL, "plain");
